+++
# --- Basic Metadata ---
id = "PRIME-DEV-RULE-ABSTRACTION-PRINCIPLE-V1"
title = "Standard: Abstraction Principle for Rules & KBs"
context_type = "rules"
scope = "Prime Config Editor guideline for structuring rules and KBs"
target_audience = ["prime-dev"] # Applies specifically to Prime Config Editor
...
**Applies To:** Prime Config Editor when creating or maintaining its rule files (`.kilocode/rules-prime-dev/`) or knowledge base articles (`.ruru/modes/prime-dev/kb/`).
...
2.  **Content Resides Externally:** Specific, detailed content such as lengthy prompts, lists of options, complex data mappings (e.g., state transitions), detailed command examples, or extensive tool usage descriptions **SHOULD** reside in separate, dedicated files. These external files can be:
    *   Knowledge Base (KB) articles (`.ruru/modes/prime-dev/kb/...`).
    *   Templates (`.ruru/templates/toml-md/...`).
    *   Dedicated prompt files (`.ruru/modes/prime-dev/kb/prompts/...` or `.ruru/docs/prompts/...`).
