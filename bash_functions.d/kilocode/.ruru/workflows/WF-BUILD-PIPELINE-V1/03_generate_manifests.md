...
description = """
Create .kilocodemodes and .kilocodemodes files for all defined build configurations
...
outputs = [
    ...
    "roomodes_manifest_path: \"{{env.BUILD_DIR}}/manifests/.kilocodemodes\"",
...
2.  **Verify Command Execution:**
    ...
    *   `- [ ]` Verify that `{{env.BUILD_DIR}}/manifests/.kilocodemodes` and `{{env.BUILD_DIR}}/manifests/.kilocodemodes` are present.
...
*   Manifest files `.kilocodemodes` and `.kilocodemodes` are generated in `{{env.BUILD_DIR}}/manifests/`.
*   The `manifest_generation_status` output is "success".
*   The `roomodes_manifest_path` output is `\"{{env.BUILD_DIR}}/manifests/.kilocodemodes\"`.
...
*   If expected output files (`{{env.BUILD_DIR}}/manifests/.kilocodemodes`, `{{env.BUILD_DIR}}/manifests/.kilocodemodes`) are not found after command execution, log the error, set `manifest_generation_status` to "failed", and proceed to `EE_handle_error.md`.