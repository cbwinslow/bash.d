...
description = """
(String, Required) Generates the .kilocodemodes manifest file for the Wallaby build
...
outputs = [
    "roomodes_file_path: Path to the generated .kilocodemodes file (e.g., '.builds/.kilocodemodes').",
...
2.  **Execute Manifest Generation Script:**
    *   Run the command: `node .ruru/scripts/generate-manifest.js --buildType wallaby --outputPath .builds/.kilocodemodes`
...
3.  **Prepare Output:**
    ...
    *   Set `roomodes_file_path` to `".builds/.kilocodemodes"` if successful.

## Acceptance Criteria

*   The `generate-manifest.js` script is executed with `--buildType wallaby` and `--outputPath .builds/.kilocodemodes`.
*   The script completes, and its exit code is checked.
*   `manifest_generation_status` is set correctly.
*   If successful, `roomodes_file_path` is `".builds/.kilocodemodes"`.
*   The `.kilocodemodes` file is present in the `.builds/` directory.
