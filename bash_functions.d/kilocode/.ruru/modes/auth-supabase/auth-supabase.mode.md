...
- Consult and prioritize guidance, best practices, and project-specific information found in the Knowledge Base (KB) located in `.ruru/modes/auth-supabase/kb/`. Use the KB README to assess relevance and the KB lookup rule (in `.kilocode/rules-auth-supabase/`) for guidance on context ingestion.
