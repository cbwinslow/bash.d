...
owner = "Roo Commander"
related_docs = [
    ".kilocode/rules-roo-commander/02-initialization-workflow-rule.md",
    ".kilocode/rules/00-user-preferences.md", # The target file
    ".ruru/modes/prime-coordinator/prime-coordinator.mode.md"
]
related_templates = []

# --- Workflow Specific Fields ---
objective = "Gather the user's desired preference changes and delegate the update of the `.kilocode/rules/00-user-preferences.md` file to the `prime-coordinator` mode."
...
## 1. Objective 🎯
*   To understand which user preferences the user wants to update in `.kilocode/rules/00-user-preferences.md` and delegate the modification task to `prime-coordinator` for safe execution.
...
*   **Step 1: Identify Desired Preference Changes (Coordinator Task)**
    ...
    *   **Procedure:**
        1.  *(Optional)* Use `read_file` to get the current content of `.kilocode/rules/00-user-preferences.md` to potentially show the user their existing settings.
        2.  Present the prompt (potentially including current settings read in step 1):
            ```xml
             ask_followup_question
              <question>Okay, let's update your preferences stored in `.kilocode/rules/00-user-preferences.md`.
    ...
    *   **Procedure:**
        ...
        3.  Formulate the delegation message:
            ...
            <new_task>
              ...
              Target File: `.kilocode/rules/00-user-preferences.md`
              Requested Changes:
              ---
              [Preference Changes]
              ---
...
*   **Step 3: Inform User & Conclude Initial Action (Coordinator Task)**
    ...
    *   **Procedure:**
        1.  Send a completion message: "Okay, I've passed your preference update request to the Prime Coordinator (`prime-coordinator`). It will handle the modification, likely asking for your confirmation before saving the changes to `.kilocode/rules/00-user-preferences.md`. Please follow its prompts."
...
*   The `prime-coordinator` mode has been delegated the task of updating the `.kilocode/rules/00-user-preferences.md` file.
