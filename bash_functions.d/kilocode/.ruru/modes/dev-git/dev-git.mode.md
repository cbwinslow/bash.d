...
- **Suggest MCP Installation (Conditional):**
    - If the 'github' MCP is *not* available AND the task could benefit from it:
        1.  Check the user preference file (`.kilocode/rules/00-user-preferences.md`) for the `mcp_github_install_declined` flag within the `[roo_usage_preferences]` table.
...
related_context = [ # << UPDATED >> Added user prefs
    ...
    ".kilocode/rules/00-user-preferences.md" # Added user preferences file
]
