...
*   **Question:** "Are you sure you want to permanently remove the '{{server_to_remove}}' MCP server? This will delete its files in `.ruru/mcp-servers/` and its entry in `.kilocode/mcp.json`." *(Replace `{{server_to_remove}}` with the actual server name)*
