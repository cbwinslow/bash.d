...
This document outlines the simplified process for installing and configuring the `vertex-ai-mcp-server` using NPM and the central `.kilocode/mcp.json` configuration file. This method replaces older approaches involving repository cloning and local `.env` files.
...
## 2. Configuration via `.kilocode/mcp.json`

All configuration for the `vertex-ai-mcp-server` is now managed centrally within the main Roo Commander MCP configuration file: `.kilocode/mcp.json`.

Locate or add the `vertex-ai-mcp-server` entry within the `servers` object in `.kilocode/mcp.json`. Configure it according to the example structure provided in the KB article:
...
After modifying `.kilocode/mcp.json`, restart Roo Commander to ensure it picks up the new configuration and attempts to connect to the Vertex AI MCP server. Check the Roo Commander logs or MCP status indicators for successful connection.
