...
related_context = [
    ...
    ".kilocode/mcp.json", # Corrected config file path
...
5.  **Gather Registration Details:**
    *   **CRITICAL:** Remind the user that this agent cannot directly modify the central configuration file (`.kilocode/mcp.json`).
...
6.  **Report to Coordinator:**
    ...
    *   State clearly: "Custom server cloned to '[TARGET_DIRECTORY_PATH]' and installation steps run. Please register the server in `.kilocode/mcp.json` with the following details: Name='[server_name]', Start Command='[start_command]', Type='[type]'." *(Replace placeholders with gathered details)*.
