...
4.  **Update `.kilocode/mcp.json`:**
    ...
    *   Construct the new server entry:
        ...
        ```
    *   Use `apply_diff` or `search_and_replace` to add/update this entry in the `servers` array within `.kilocode/mcp.json`. Ensure valid JSON.
