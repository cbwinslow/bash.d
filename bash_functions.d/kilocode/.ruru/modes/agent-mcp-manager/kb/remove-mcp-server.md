...
success_criteria = [
    ...
    "Server entry is removed from `.kilocode/mcp.json`.",
    "Agent reports successful removal."
    ]
failure_criteria = [
    "Agent cannot read/parse `.kilocode/mcp.json`.",
    ...
    "Agent fails to update `.kilocode/mcp.json`."
    ]
...
## 1. Objective 🎯
Safely remove an installed MCP server by deleting its directory and removing its configuration entry from `.kilocode/mcp.json` after user confirmation.

## 2. Preconditions🚦
*   `.kilocode/mcp.json` exists.

## 3. Procedure Steps 🪜

1.  **Read Configuration & List Servers:**
    *   Use the `read_file` tool to load `.kilocode/mcp.json`.
...
5.  **Update MCP Configuration (`.kilocode/mcp.json`):**
    *   Inform the user: "Removing '{{server_to_remove}}' from `.kilocode/mcp.json`..." (replace placeholder).
    ...
    *   Use the `write_to_file` tool to save the modified JSON object back to `.kilocode/mcp.json`. Ensure proper formatting (e.g., using `JSON.stringify(data, null, 2)`).
