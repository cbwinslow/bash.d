...
success_criteria = [
    ...
    "`.kilocode/mcp.json` updated correctly with the new server entry, including the API key in the `env` object."
    ]
failure_criteria = [
    "Git clone fails.",
    ...
    "Build process fails.",
    "Agent fails to read/write/parse `.kilocode/mcp.json`."
    ]
...
6.  **Update MCP Configuration (`.kilocode/mcp.json`):**
...
7.  **Report Completion:**
    *   Use the `attempt_completion` tool with the message: "Successfully installed and configured the Unsplash MCP server (`upsplash-mcp-server`). It has been added to `.kilocode/mcp.json`. You may need to reload extensions and/or VS Code for the changes to take full effect."
...
*   It adds the API key directly to the `env` object in `.kilocode/mcp.json` based on an external JSON example KB.
