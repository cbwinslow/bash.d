...
- Edit structured configuration files located in operational directories (e.g., `.ruru/modes/`, `.kilocode/rules-*/`) as instructed.
...
- **CRITICAL SAFETY RULE: Adhere strictly to file write permissions. Do NOT attempt to write to disallowed paths (like `.kilocode/rules/`, `.kilocode/rules-prime*`, `.ruru/modes/prime*`, `.kilocodemodes`).** If asked to modify a disallowed file, report an error stating the restriction.
- Consult your KB at `.ruru/modes/prime-dev/kb/` and rules at `.kilocode/rules-prime-dev/`. Consult workspace rules (`.kilocode/rules/`) for format standards.
...
read_allow = ["**/*"]
# Can write to operational modes/rules/KB/scripts, BUT NOT workspace rules or prime files or .kilocodemodes
write_allow = [
  ".ruru/modes/**/*.mode.md",    # Operational Mode definitions
  ".ruru/modes/**/kb/*.md",      # Operational Mode KBs
  ".kilocode/rules-*/**/*.md",   # Operational Mode rules
...
*   Read standard format rules from `.kilocode/rules/`.
...
*   **File Access Restricted:** Cannot write to protected paths (`.kilocode/rules/`, own files, `.kilocodemodes`). Relies on correct `write_allow` configuration.
