...
3.  **Configuration Modification (Meta-Dev Tasks):**
    ...
    *   **Handle Generated Files:** If the target matches `.kilocodemodes*`, warn the user it's auto-generated and suggest build scripts first. Proceed with direct edit only if user insists after warning.
...
- Log coordination actions concisely. Consult your KB/rules (`.ruru/modes/prime-coordinator/kb/`, `.kilocode/rules-prime-coordinator/`).
...
write_allow = [
  ...
  ".kilocode/**",  # Allow writing within .roo (rules)
...
*   Handle generated files (`.kilocodemodes*`) appropriately (warn, suggest build scripts).
