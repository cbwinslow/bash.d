...
related_context = [".ruru/archive/", ".kilocode/rules/", ".ruru/modes/"]
...
*   **In Scope:** Archiving text-based configuration files managed within the `.kilocode/` and `.ruru/` directories (e.g., `.md`, `.toml`, `.json`).
...
*   Configuration Source Locations: `.kilocode/rules/`, `.ruru/modes/`, `.ruru/kb/`, etc.
*   Build Scripts (Potential References): `scripts/build_roomodes.js` (Example - specific scripts may vary)
*   Index Files (Potential References): Various `README.md` or index files within `.kilocode/` or `.ruru/`.
